...
(function ($) {
	...
	$(document).ready(function () {
		...
		if ($(window).width() > 768) {
			...
			$('.electro-animate-dropdown, .departments-menu-v2, .cat-nav .mas-elementor-nav-menu').on('mouseleave', function () {
				var $this = $(this);
				$this.removeClass('animated-dropdown');
			});

			$('.electro-animate-dropdown .menu-item, .departments-menu-v2 .menu-item, .cat-nav .handheld .menu-item').on('mouseenter', function () {
				var $this = $(this),
					$departments_menu = $this.parents('.departments-menu-v2'),
					$cat_nav_handheld = $this.parents('.cat-nav .handheld'),
					$container = $this.parents('.electro-animate-dropdown');

				if ($departments_menu.length > 0) {
					$container = $departments_menu;
				} else if ($cat_nav_handheld.length > 0) {
					$container = $cat_nav_handheld;
				}
